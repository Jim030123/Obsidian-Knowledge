...
var require_decode_data_xml = __commonJS({
  "node_modules/entities/lib/generated/decode-data-xml.js"(exports2) {
    ...
    init_buffer_polyfill();
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.default = new Uint16Array("\u0200aglq	\x1B\u026D\0\0p;\u4026os;\u4027t;\u403Et;\u403Cuot;\u4022".split("").map(function(c) {
      return c.charCodeAt(0);
    }));
  }
});
...
var require_src = __commonJS({
  "node_modules/protobufjs/src/index.js"(exports2, module2) {
    ...
    protobuf2.parse = require_parse4();
...
function generateTranscriptParams(videoId, useAsrStyle, field6Value, lang = "en") {
...
function generateAlternativeTranscriptParams(videoId, lang = "en") {
  const variations = [
  ...
  return variations.map((variant) => generateTranscriptParams(videoId, variant.useAsrStyle, variant.field6Value, lang));
}
...
function extractParamsFromPage(htmlContent) {
  console.log("\u{1F50D} DEBUG: Looking for ytInitialData script tag...");
  console.log("\u{1F4CA} DEBUG: HTML content length:", htmlContent.length, "characters");
  let ytInitialDataMatch = htmlContent.match(/var ytInitialData\s*=\s*({.+?});/s);
  if (!ytInitialDataMatch) {
    console.log("\u274C DEBUG: No ytInitialData script found with 'var' pattern");
    const altPatterns = [
    ...
    for (let i = 0; i < altPatterns.length; i++) {
      ...
      if (match) {
        console.log(`\u2705 DEBUG: Found ytInitialData using alternative pattern ${i + 1}`);
  ...
  try {
    let findGetTranscriptEndpoint = function(obj, path = "", depth = 0) {
      if (!obj || typeof obj !== "object") {
        return null;
      }
      if (depth === 0) {
        console.log("\u{1F50D} DEBUG: Starting recursive search for getTranscriptEndpoint...");
      }
      if (obj.getTranscriptEndpoint && obj.getTranscriptEndpoint.params) {
        console.log(`\u2705 DEBUG: Found getTranscriptEndpoint.params at path: ${path} (depth: ${depth})`);
        console.log(`\u{1F4CF} DEBUG: Params length: ${obj.getTranscriptEndpoint.params.length} characters`);
        return obj.getTranscriptEndpoint.params;
      }
      if (depth < 3) {
        for (const key of Object.keys(obj)) {
          if (key.toLowerCase().includes("transcript") || key.toLowerCase().includes("engagement")) {
            console.log(`\u{1F50D} DEBUG: Found transcript/engagement-related key: "${key}" at path: ${path} (depth: ${depth})`);
          }
        }
      }
    ...
    console.log("\u{1F527} DEBUG: Parsing ytInitialData JSON (length:", ytInitialDataString.length, "chars)");
    ...
    if (params && typeof params === "string" && params.length > 50) {
      console.log(`\u2705 DEBUG: Found valid getTranscriptEndpoint params (${params.length} chars):`, params.substring(0, 50) + "...");
      return params;
    } else if (params) {
      console.log(`\u26A0\uFE0F DEBUG: Found getTranscriptEndpoint params but too short (${params.length} chars):`, params);
    } else {
      console.log("\u274C DEBUG: No getTranscriptEndpoint.params found in ytInitialData");
    }
  } catch (error) {
...
function extractVisitorData(htmlContent) {
  const visitorDataMatch = htmlContent.match(/"visitorData"\s*:\s*"([^"]+)"/) || htmlContent.match(/visitorData['"]\s*:\s*['"]([^"']+)['"]/);
  if (visitorDataMatch) {
    console.log("\u2705 DEBUG: Found visitorData:", visitorDataMatch[1].substring(0, 20) + "...");
...
function parseVideoPageWithFallbacks(htmlContent, config) {
  ...
  console.log("\u{1F50D} DEBUG: Params found on video page:", pageParams || "NOT FOUND");
  if (pageParams) {
    console.log("\u{1F4CF} DEBUG: Page params length:", pageParams.length, "characters");
    console.log("\u{1F50D} DEBUG: Page params preview:", pageParams.substring(0, 100) + "...");
  }
  const generatedParams = generateAlternativeTranscriptParams(videoId, (config == null ? void 0 : config.lang) || "en");
  console.log("\u{1F527} DEBUG: Generated", generatedParams.length, "params variations for video ID:", videoId);
  generatedParams.forEach((params, index) => {
    console.log(`  Generated variation ${index + 1} (${params.length} chars):`, params.substring(0, 50) + "...");
  });
  if (pageParams) {
    let foundMatch = false;
    generatedParams.forEach((generatedParam, index) => {
      if (generatedParam === pageParams) {
        console.log(`\u{1F3AF} MATCH! Page params identical to generated variation ${index + 1}`);
        foundMatch = true;
      }
    });
    if (!foundMatch) {
      console.log("\u{1F504} DEBUG: Page params are DIFFERENT from all generated variations - this could be video-specific!");
    }
  }
  const allParams = pageParams ? [pageParams, ...generatedParams] : generatedParams;
  if (pageParams) {
    console.log("\u2705 DEBUG: Will try page params FIRST (attempt 1), then", generatedParams.length, "generated variations");
  } else {
    console.log("\u26A0\uFE0F DEBUG: No valid page params found, will try", generatedParams.length, "generated variations only");
  }
  const transcriptRequests = allParams.map((params) => {
    const requestBody = {
      context: {
        client: {
          ...
          gl: (config == null ? void 0 : config.country) || "EN",
    ...
    return {
      ...
      headers: {
        ...
        "X-Goog-EOM-Visitor-Id": visitorData || "Cgs5LXVQa0I1YnhHOCjZ7ZDDBjInCgJQTBIhEh0SGwsMDg8QERITFBUWFxgZGhscHR4fICEiIyQlJiAS",
...
var YoutubeTranscript = class {
  static async getTranscript(url, config) {
    try {
      ...
      console.log(`\u{1F680} DEBUG: Starting transcript fetch with ${videoData.transcriptRequests.length} different parameter combinations`);
      for (let i = 0; i < videoData.transcriptRequests.length; i++) {
        ...
        try {
          console.log(`\u{1F3AF} Attempt ${i + 1}/${videoData.transcriptRequests.length}: Trying ${paramsSource} params: ${paramsInfo}`);
